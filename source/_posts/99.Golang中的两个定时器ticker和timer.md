...
{% note info %}
 ...
 本文转自 [Blog](https://blog.haohtml.com/archives/19859)
...
__1. 这里需要注意的时，如果在调用 `time.Reset() `或 `time.Stop() `的时候，timer已经过期或者停止了，则会返回`false`。__
...
__3. 使用 `time.NewTicker()` 定时器时，需要使用 `Stop()` 方法进行资源释放，否则会产生内存泄漏，(`Stop the ticker to release associated resources`.)__
