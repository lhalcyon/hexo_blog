...
date: 2022/6/2
updated: 2022/6/2
urlname: mysql-encrypt-mybatis-typehandler
tags: [Mybatis]
categories: 
- Java
---
...
本文转自 [github.io](https://vazmin.github.io/java/mybatis-secret-string-type-handler/)
...
2. DTO层手动处理。
3. 编写拦截器插件，定义加密注解，监听`ParameterHandler`，`ResultSetHandler` 信号对带加密注解的参数进行加密,带加密注解的字段解密。 （尝试过，存在不能容忍的弊端,例如插入对象后，需要加密字段的值被加密，后续代码要使用还得解密。 不推荐使用）
